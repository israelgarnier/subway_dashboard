...
// --- Meltwater: menciones de la búsqueda "Compra subway" ---
const MW_ACQ_RE = /(coprocom|subs empire|subs cfa|grupo subs|franquicia maestra|adquisic|adquir|traspaso|compra de la franquicia|venta de la franquicia|cambio de due[ñn]o|roark|comprar franquicia|concentraci[oó]n econ[oó]mica)/i;

function meltwaterRelevant(text) {
  const t = plain(text);
  if (!MW_ACQ_RE.test(t)) return false;
  return /subway|franquicia|comida[s]? r[aá]pida/i.test(t);
}

export function processMeltwater(documents) {
  const seen = new Set();
  const all = [];
  for (const d of documents || []) {
    ...
    seen.add(url);
    const c = d.content || {};
    const title = c.title || c.opening_text || "";
    all.push({
      title,
      publisher: (d.source && d.source.name) || "",
      domain: (d.source && d.source.domain) || "",
      url,
      date: toISO(d.published_date || d.indexed_date),
      reach: (d.source && d.source.metrics && d.source.metrics.reach) || 0,
      type: d.content_type || "",
      relevant: meltwaterRelevant(title),
    });
  }
  const relevant = all
    .filter((m) => m.relevant)
  ...
  return {
    ...
    totalReach: relevant.reduce((s, m) => s + (m.reach || 0), 0),
    mentions: relevant,
  };
}
